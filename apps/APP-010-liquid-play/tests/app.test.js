...
assert.ok(state.holdSignal.innerRadius >= 38 && state.holdSignal.outerRadius >= 66, "calling rings extend beyond a typical fingertip area");
...
assert.ok(Math.min(...holdArcs.map((call) => call[3])) >= 38 && Math.max(...holdArcs.map((call) => call[3])) >= 66, "rendered signal remains visible outside the finger area");
...
assert.equal(expressionEnv.drawCalls.filter(([name]) => name === "quadraticCurveTo").length, 3, "resting frame draws one smile path per jelly");
...
expressionEnv.drawCalls.length = 0;
expressionEnv.step(1);
assert.equal(expressionEnv.drawCalls.filter(([name]) => name === "quadraticCurveTo").length, 3, "intermediate expression still draws one mouth path per jelly");
...
expressionEnv.drawCalls.length = 0;
expressionEnv.step(1);
assert.equal(expressionEnv.drawCalls.filter(([name]) => name === "quadraticCurveTo").length, 3, "active frame draws only one transformed mouth per jelly");
