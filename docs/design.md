...
- Canvasと少数の液体玉を使用し、外部の重量級物理・流体ライブラリはMVPへ追加しない。
- 傾き入力と長押し入力を共通の力ベクトルへ正規化し、同じ更新ループへ渡す。
- センサー値は上限、デッドゾーン、平滑化を適用し、急激な跳ねや異常値を避ける。
- センサー非対応・拒否・取得失敗時は長押し入力へ安全にフォールバックする。
- `requestAnimationFrame`は1本だけ維持し、非表示時に停止、復帰時に二重起動させない。
- 画面回転・リサイズ時は座標と境界を再計算し、液体玉を表示領域内へ補正する。
- 変形は速度方向の伸縮や接触時の軽い潰れに限定し、合体・分裂判定は実装しない。
- reduced-motion時は移動速度、慣性、変形量を弱める。
- センサー値、タッチ位置、液体位置を保存または分析サービスへカスタム送信しない。
...
- 測定ID `G-M5FPMG34LE`を`shared/ga4.js`の1か所で管理する。
- Repositoryルートは`shared/ga4.js`、正式公開APPは`../../shared/ga4.js`を参照し、GitHub PagesのURLパスを維持する。
- Google tagは非同期で読み込み、取得失敗や遮断時もポータルと各アプリの初期化・操作へ依存関係を作らない。
- 標準ページ計測だけを使用し、カスタムイベント、カスタムユーザーID、入力内容、センサー値、タッチ位置を送信しない。
- 初期化フラグと同一URLのscript確認により、同じページでGoogle tagを二重読込しない。
- 将来正式公開するAPPも、公開時に同じ共通ファイルを1回だけ参照する。
